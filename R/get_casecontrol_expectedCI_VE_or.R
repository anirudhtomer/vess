#' Expected lower and expected upper limit of the confidence intervals of variant and vaccine-specific efficacy based on odds ratio.
#' @description
#' The function `get_cohort_expectedCI_VE_or` simulates confidence intervals for variant and vaccine-specific efficacy (VE) for a given sample size.
...
#' @export
get_casecontrol_expectedCI_VE_or = function(anticipated_VE_for_each_brand_and_variant=
  ...
  cell_counts_case_sims = array(data = do.call('cbind', lapply(missing_data_adjusted_total_cases, rmultinom, n=nsims,
                                ...
                                dim = c((total_vaccine_brands+1)*total_case_variants, nsims, total_total_case_settings))
  ...
  cell_counts_control_sims = array(data = do.call('cbind', lapply(missing_data_adjusted_total_controls, rmultinom, n=nsims,
                                                                  prob=case_control_full_tables$conditional_vector_controls)),
  ...
  brand1_case_indices = brand1_control_indices + (relative_VE_combn[VARIANT,]-1)*nrow(case_control_full_tables$full_table_cases)
  brand2_control_indices = relative_VE_combn[BRAND2,] + UNVACCINATED_ROW
  brand2_case_indices =  brand2_control_indices + (relative_VE_combn[VARIANT,]-1)*nrow(case_control_full_tables$full_table_cases)
  ...
  flat_conditional_table_cases = c(case_control_full_tables$conditional_table_cases)
  odds_vaccinated_vaccine1 = flat_conditional_table_cases[brand1_case_indices]/case_control_full_tables$conditional_vector_controls[brand1_control_indices]
  odds_vaccinated_vaccine2 = flat_conditional_table_cases[brand2_case_indices]/case_control_full_tables$conditional_vector_controls[brand2_control_indices]
