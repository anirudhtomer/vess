#' Minimum detectable variant and vaccine-specific efficacy based on incidence rate ratio.
#' @description
#' The function `get_cohort_mindet_VE_irr` calculates the minimum detectable variant and vaccine-specific efficacy (VE) for a given sample size, power, and type-I error rate.
...
#' The effective incidence rate of the variant in this comparison is the variant-specific incidence rate of the variant taken
...
#' @export
get_cohort_mindet_VE_irr = function(anticipated_VE_for_each_brand_and_variant=
  ...
  mindet_VE = t(apply(relative_VE_combn, MARGIN = 2, function(comparison_set){
    ...
    event_rate_comparison_group = event_rates_table[comparison_set[VARIANT], comparison_set[BRAND2] + UNVACCINATED_COL]
